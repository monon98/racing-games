...
- 底盘：`Box(0.95, 0.36, 2.05)`，质量 550，线性阻尼 0.25、角阻尼 0.7（低重心、高离地，防刹车点头前翻）。
- 四轮：半径 0.35，悬架刚度 45、restLength 0.38、行程 0.25、压缩阻尼 6.5、回弹阻尼 4.0、frictionSlip 2.1、rollInfluence 0.08；前轮 z=+1.554、后轮 z=-1.554，x=±1.12（轮距外扩），连接点 y=-0.24。
- 控制：`throttle` 施加于后轮（±3200N，正向取反），`brake` 施加四轮（≤70），转向最大 0.55rad、按 6/s 平滑；悬架压缩阻尼 9.5（抗刹车点头）。
- 倒车限制：加速力 ×0.5（约 5.8 m/s²），最高倒车速度 8 m/s；松油门且不踩刹车时施加 60N 发动机制动。

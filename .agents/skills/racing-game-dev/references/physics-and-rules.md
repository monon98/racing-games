...
- 底盘：`Box(0.95, 0.36, 2.05)` 下偏 0.08m，质量 550，线性阻尼 0.12、角阻尼 0.7（低重心、高离地）。
- 四轮：半径 0.45，悬架刚度 45、restLength 0.38、行程 0.25、压缩阻尼 9.5、回弹阻尼 4.0、frictionSlip 3.2、rollInfluence 0.08；前轮 z=+1.554、后轮 z=-1.554，x=±1.15（轮距外扩），连接点 y=-0.24。
- 控制：`throttle` 施加于后轮（±3500N，正向取反，0-100km/h 约 2.5s），`brake` 施加四轮（≤55），转向最大 0.65rad、按住按 7/s 打角、松键按 3/s 缓慢回中。
- 倒车限制：加速力 ×0.7，最高倒车速度 8 m/s；松油门且不踩刹车时施加 35N 发动机制动。

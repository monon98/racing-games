...
- 滑行制动：单纯松油门（无转向）用 COAST_BRAKE=4 缓慢滑行；松油门但按住左右键用 ENGINE_BRAKE=50 快速降速；按后退键（倒车输入）在前进时按刹车处理，急速降速。
...
- 空中钳制：任一车轮离地时上升速度 ≤ 1.5 m/s；**四轮全离地**时上升速度 ≤ 0.6 m/s 并施加约 3.2g 下压力，地形山脊/过顶不飞射。

...
- `generateCenterlinePoints(meta)`：
  ...
  - 复杂模式（2026-08-14 重做）：随机 **M×N 高度矩阵**（M 行 × N 列，行列随机）作为网格高度场，再用**哈密顿路径**在网格上连成一条闭环中心线（生成快、不再卡死）；相邻网格高度差限制在可行驶范围（最终再按采样点坡度 ≤12~13% 钳制，避免断崖）；**目前不考虑交叉轨道场景**，高度矩阵只负责升降起伏，不强制交叉净空。

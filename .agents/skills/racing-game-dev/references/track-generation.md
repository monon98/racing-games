...
- `generateCenterlinePoints(meta)`：
  ...
  - 复杂模式（2026-08-14 地形优先重做）：**先生成覆盖赛道的 2D 高度图地形**（`generateTerrain`：值噪声 + 平滑插值 + 相邻格高差钳制 ≈12%，并强制一个山丘保证必然有升降），**再画 2D 闭环轨道**（复用星形控制点 + Catmull-Rom，不自交）；轨道逐点双线性采样地形高度，最后按采样点坡度 ≤12~13% 钳制；**不考虑交叉轨道**。

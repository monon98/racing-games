...
## 路面与护栏
- 路面横向右向量 = `(t.z, 0, -t.x)` 归一化；两侧偏移 = `roadWidth / 2`。
- 路面 BufferGeometry：每个采样点两个顶点（左/右），闭合带状三角面（法线向上），存入 `roadGeometry` 供物理 Trimesh 与导出复用。
- 起终点线：白色薄盒横跨路面，位于 `points[0]`，按切线朝向旋转。
- 护栏：每 3 个采样点一段，`BoxGeometry(BARRIER_THICKNESS, barrierHeight, segLen)` 置于路面外侧 0.08m；视觉与 cannon-es 静态 Box 一一对应。
